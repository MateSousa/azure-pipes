...
variable "aliases" {
  type = list(object({
    ...
    locale_id   = optional(string, "en_US")
  }))
  description = "List of bot aliases. If bot_version is null and bot_version.create is true, uses the created version. lambda_arn associates a Lambda function for fulfillment on the alias."
  default     = []
}
...
variable "connect_integration" {
  type = object({
    ...
    connect_instance_arn = optional(string, null)
    account_id           = optional(string, null)
  })
  description = "Connect integration — adds resource policy allowing Connect to invoke the bot aliases."
  default = {
    enabled = false
  }
}
