...
variable "table" {
  ...
  validation {
    condition     = var.table.billing_mode != "PROVISIONED" || var.table.write_capacity != null
    error_message = "table.write_capacity is required when billing_mode is PROVISIONED."
  }
  ...
  validation {
    condition     = var.table.stream_enabled || var.table.stream_view_type == null
    error_message = "table.stream_view_type must be null when stream_enabled is false."
  }

  validation {
    condition     = !var.table.stream_enabled || var.table.stream_view_type != null
    error_message = "table.stream_view_type is required when stream_enabled is true."
  }
}
...
variable "global_secondary_indexes" {
  ...
  validation {
    condition = alltrue([
      ...
      idx.projection_type != "INCLUDE" || length(coalesce(idx.non_key_attributes, [])) > 0
    ])
    error_message = "non_key_attributes is required when projection_type is INCLUDE for global secondary indexes."
  }
}
...
variable "local_secondary_indexes" {
  ...
  validation {
    condition = alltrue([
      for idx in var.local_secondary_indexes :
      idx.projection_type != "INCLUDE" || length(coalesce(idx.non_key_attributes, [])) > 0
    ])
    error_message = "non_key_attributes is required when projection_type is INCLUDE for local secondary indexes."
  }
}
