...
variable "policies" {
  ...
  validation {
    condition     = alltrue([for arn in var.policies.managed_policy_arns : can(regex("^arn:aws:iam::(\\d{12}|aws):policy/.+$", arn))])
    error_message = "Each managed_policy_arns entry must be a valid IAM policy ARN (customer-managed arn:aws:iam::<account-id>:policy/<name> or AWS-managed arn:aws:iam::aws:policy/<name>)."
  }
}
