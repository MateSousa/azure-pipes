...
variable "environment" {
  description = "Environment name (e.g. dev, qa, prod). Used in resource naming."
  type        = string
}
...
variable "contact_flows" {
  ...
  validation {
    condition     = alltrue([for cf in var.contact_flows : cf.content_file != null || cf.content != null])
    error_message = "Each contact flow must specify either content_file or content."
  }
}
...
variable "contact_flow_modules" {
  ...
  validation {
    condition     = alltrue([for cfm in var.contact_flow_modules : cfm.content_file != null || cfm.content != null])
    error_message = "Each contact flow module must specify either content_file or content."
  }
}
