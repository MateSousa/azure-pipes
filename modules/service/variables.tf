...
variable "service" {
  ...
  validation {
    condition     = var.service.launch_type == null ? true : contains(["FARGATE", "EC2"], var.service.launch_type)
    error_message = "launch_type must be FARGATE, EC2, or null (to use cluster capacity provider strategy)."
  }
}
...
variable "ecs" {
  ...
  type = object({
    ...
    task_definition_family   = optional(string, null)
    task_definition_revision = optional(number, 0)
