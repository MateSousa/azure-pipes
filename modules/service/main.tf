locals {
  ...
  # Always use the latest task definition revision, whether it came from
  # Terraform (env var change) or CI/CD (new image deploy).
  # Compare revisions and pick the full ARN from whichever is newer.
  task_definition_latest = var.ecs.task_definition_family != null ? (
    var.ecs.task_definition_revision >= data.aws_ecs_task_definition.latest[0].revision
    ? var.ecs.task_definition_arn
    : data.aws_ecs_task_definition.latest[0].arn
  ) : var.ecs.task_definition_arn
}

# Fetch the latest active revision so CI/CD deploys are not reverted
data "aws_ecs_task_definition" "latest" {
  count           = var.ecs.task_definition_family != null ? 1 : 0
  task_definition = var.ecs.task_definition_family
}

resource "aws_ecs_service" "this" {
  name                   = "${var.project_name}-${var.service.name}"
  cluster                = var.ecs.cluster_id
  task_definition        = local.task_definition_latest
  desired_count          = var.service.desired_count
  launch_type            = var.service.launch_type
  platform_version       = var.service.launch_type == "FARGATE" || var.service.launch_type == null ? var.service.platform_version : null
  ...
  lifecycle {
    precondition {
      condition     = var.load_balancer == null || var.ecs.container_port != null
      error_message = "ecs.container_port is required when load_balancer is configured."
    }
  }
